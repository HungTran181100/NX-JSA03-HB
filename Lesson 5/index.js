// let prod = [
//   { name: "Prod1", price: 24, des: "ABC" },
//   { name: "Prod2", price: 24, des: "DEFG" },
//   { name: "Prod3", price: 24, des: "HIJK" },
// ];
...
//   h1.innerHTML = prod[i].name;
//   h2.innerHTML = prod[i].price;
//   p.innerHTML = prod[i].des;
...
function getProducts() {
  for (let i = 0; i < products.length; i++) {
    ...
    <div class="col-md-3 cardItem">
        ...
        <div class="card-body">
            <h5 class="card-title productName">${products[i].productName}</h5>
...
search.addEventListener("submit", (event) => {
  ...
  productName.forEach((items, index) => {
    if (items.innerText.includes(searchInput)) {
      cardItem[index].classList.remove("hide");
    } else {
      cardItem[index].classList.add("hide");
    }
  });
});
